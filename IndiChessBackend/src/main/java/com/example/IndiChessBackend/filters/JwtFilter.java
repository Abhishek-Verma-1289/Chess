...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        logger.trace("Inside jwt filter");
        if (token != null) {
            logger.trace("JWT token found (masked) : {}", token.replaceAll(".(?=.{4})", "*") );
        }
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                ...
                logger.trace("Username extracted from token: {}", username);
                ...
                // User from token not found in DB; skip authentication
                logger.debug("User not found for username extracted from token: {}", username);
            }
        }
